...
namespace HRUTWeb
{
    public class FaceDrawer
    {
        ...
        private Image arrow = Image.FromFile(@".\arrow\1.png");

        public FaceDrawer()
        {
            pfc.AddFontFile(@".\fonts\2.ttf");
        }
        ...
        private void DrawIndex(Graphics g, FaceBase face, Size size)
        {
            ...
            g.DrawOutlineString(etext, font, emotionColor, Color.Black, loc);
#if DEBUG
            ...
            int pid = DirectoryHelper.RandomFile(@".\progress");
            using (Image progess = Image.FromFile($@".\progress\{pid}.png"))
            using (Image progessx = Image.FromFile($@".\progressx\{pid}.png"))
            {
